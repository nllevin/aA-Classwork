...
function sumArray(array) {
    ...
    return copy[0] + sumArray(copy.slice(1, copy.length));
}
...
function reverseString(str) {
    ...
    return reverseString(copy.slice(1, copy.length)) + copy[0];
}
...
function flatten(data) {
    ...
    return data.reduce((flattened, ele) => flattened.concat(flatten(ele)), []);
}
...
function fileFinder(directories, targetFile) {
    const topDirs = Object.keys(directories);
    if (topDirs.includes(targetFile)) return true;
    if (topDirs.every(dir => directories[dir] === null)) return false;
    return topDirs.some(dir => (
        directories[dir] && fileFinder(directories[dir], targetFile)
    ));
}
...
function pathFinder(directories, targetFile) {
    ...
    for (let i = 0; i < topDirs.length; i++) {
        ...
        const nestedDirs = directories[topDir];
        if (topDir === targetFile) {
            return `/${targetFile}`;
        } else if (nestedDirs) {
            const subPath = pathFinder(nestedDirs, targetFile);
            if (subPath) return topDir + subPath;
        }
    }
    return null;
}
